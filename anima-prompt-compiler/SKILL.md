...
description: Compile character concepts, fashion designs, OC settings, composition briefs, and visual blueprints into practical Anima prompts using a disciplined Tag + Natural Language format and an evolving user aesthetic profile.
---
...
- 构图与景别：`full body`, `upper body`, `portrait`, `white background`
- 外观：发色、发型、瞳色、种族特征
- 服装单品：帽子、外套、胸衣、裙子、披风、靴子等
- 道具与动作：`holding sword`, `shield`, `looking at viewer`
...
用户偏好的核心不是“越复杂越好”，而是**好看的服装造型、成熟的搭配逻辑和具有识别度的角色轮廓**。
...
- 泛化的“神秘、优雅、复杂、华丽”形容词堆叠
- 只有主题名称，没有具体服装剪裁
- 任意添加机械、蝴蝶、玫瑰、魔法阵、荧光粒子等常见装饰
- 全身平均铺满饰品
- 不同概念之间缺乏关系的元素拼接
- 先写一堆配饰，再强行补动作
- 用光影和质量词掩盖造型不成立
...
## 6. Dynamic User Aesthetic Profile
